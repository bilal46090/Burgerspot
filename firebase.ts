...
async function testConnection() {
  try {
    // If the database is new, system/ping might not exist. 
    // We'll try to get ANY doc from a common collection or just use a dummy path.
    // The specific error determines connectivity.
    const testDoc = doc(db, 'system', 'connectivity_check');
    
    // We don't care if the doc exists or we have permission, 
    // we only care if the server responds at all.
    await getDocFromServer(testDoc).catch(e => {
       // These codes mean we actually REACHED the server
    ...
    if (isOffline) {
      ...
      // If we got some other error (like a weird internal error), we might still be connected
      // but let's be safe and not toggle status unless we are sure it's a network issue.
      console.warn("ℹ️ Firestore Status Check returned unexpected error (likely connected):", error.code || error.message);
      // We don't necessarily set to false here to avoid flickering if it's just a permissions issue on the ping doc
    }
  }
}
testConnection();
// Periodic check for reconnection
setInterval(testConnection, 15000); // 15 seconds for faster recovery feedback
